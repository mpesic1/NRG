...
Potree.Renderer = class Renderer {
	...
	renderOctree(octree, nodes, camera, target, params = {}){
		...
		{ // UPDATE UNIFORMS
			...
			shader.setUniform1f("snowHeight", material.snowHeight);
			shader.setUniform1f("windSpeed", material.windSpeed);
			shader.setUniform1f("frameNum", material.frameNum);
...
vec3 getRGB(){
	...
	if((classification == 3.0 || classification == 2.0|| classification == 1.0|| classification == 0.0) && snowHeight > 0.0) {			// snow color
		rgb.x = 1.0;
		rgb.y = 1.0;
		rgb.z = 1.0;
	}
...
void main() {
	vClassification = classification;																				// pass to fragment shader
	vec4 mvPosition = modelViewMatrix * vec4( position, 1.0 );
	
	if(classification == 3.0 || classification == 2.0) {
		mvPosition.y += snowHeight; 																				//	snow height	
	}


	if(windSpeed > 0.0 && classification == 5.0) {																	// wind
		mvPosition.x += (windSpeed / 10.0) * sin((frameNum + mvPosition.y) / 10.0);
		mvPosition.z += (windSpeed / 15.0) * cos((frameNum + mvPosition.y) / 8.0);
	}
...
void main() {
	...
	//if((vClassification == 3.0 || vClassification == 2.0|| vClassification == 1.0|| vClassification == 0.0) && snowHeight > 0.0) {			// snow color
	//	color.x = 1.0;
	//	color.y = 1.0;
	//	color.z = 1.0;
	//}
...
Potree.PointCloudMaterial = class PointCloudMaterial extends THREE.RawShaderMaterial {
	constructor (parameters = {}) {
		...
		this.uniforms = {
			windSpeed: 			{ type: 'f', value: 0.0 },
			frameNum: 			{ type: 'f', value: 0.0 },
			snowHeight: 		{ type: 'f', value: 0.0 },
	...
	get snowHeight () {
		return this.uniforms.snowHeight.value;
	}

	set snowHeight (value) {
		if (this.uniforms.snowHeight.value !== value) {
			this.uniforms.snowHeight.value = value;
			this.dispatchEvent({
				type: 'material_property_changed',
				target: this
			});
	...
	get windSpeed () {
		return this.uniforms.windSpeed.value;
	}

	set windSpeed (value) {
		if (this.uniforms.windSpeed.value !== value) {
			this.uniforms.windSpeed.value = value;
			this.dispatchEvent({
				type: 'material_property_changed',
				target: this
			});
	...
	get frameNum () {
		return this.uniforms.frameNum.value;
	}

	set frameNum (value) {
		if (this.uniforms.frameNum.value !== value) {
			this.uniforms.frameNum.value = value;
			this.dispatchEvent({
				type: 'material_property_changed',
				target: this
			});
	...
	render () {
		...
		{ // ATTRIBUTE PASS
			for (let pointcloud of visiblePointClouds) {
				...
				attributeMaterial.snowHeight = material.snowHeight;
				attributeMaterial.windSpeed = material.windSpeed;
				attributeMaterial.frameNum = material.frameNum;
	...
	render () {
		...
		{ // THREEJS
			...
			for (let [pointcloud, entry] of this.pointclouds) {
				...
				material.snowHeight = pointcloud.material.snowHeight;
				material.windSpeed = pointcloud.material.windSpeed;
				material.frameNum = pointcloud.material.frameNum;
